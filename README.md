...
If Telegram revokes the session while the server is running, the first call the account's home data centre refuses stops the client. File downloads and statistics calls can be served by another data centre, whose refusal says nothing about the session: that call fails at once, and the server asks the home data centre in the background. If the home data centre refuses the session too, the client stops as above; if it still accepts it, the client stops with a reason saying that reconnecting restores those calls. From then on every tool call answers with the reason and fix instead of reaching Telegram, and a call the stop cut short gets them appended to its own result — a call that completed is told its result stands. The host stays connected, and after `mcp-telegram login` (for a refused session) a reconnect loads the tools again. A client that stops for any other reason, such as a permanent connection failure, is reported the same way, and reconnecting the server recovers it.
